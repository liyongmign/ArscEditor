...
package com.mly.arsc.decode;

import com.mly.arsc.decode.data.ResConfigFlags;
import com.mly.arsc.decode.data.ResID;
import com.mly.arsc.decode.data.ResPackage;
import com.mly.arsc.decode.data.ResResSpec;
import com.mly.arsc.decode.data.ResResource;
import com.mly.arsc.decode.data.ResTable;
import com.mly.arsc.decode.data.ResType;
import com.mly.arsc.decode.data.ResTypeSpec;
import com.mly.arsc.decode.data.value.ResBagValue;
import com.mly.arsc.decode.data.value.ResBoolValue;
import com.mly.arsc.decode.data.value.ResFileValue;
import com.mly.arsc.decode.data.value.ResScalarValue;
import com.mly.arsc.decode.data.value.ResStringValue;
import com.mly.arsc.decode.data.value.ResValue;
import com.mly.arsc.decode.data.value.ResValueFactory;
import com.mly.arsc.decode.io.Duo;
import com.mly.arsc.decode.io.LEDataInputStream;
import com.mly.arsc.decode.io.LEDataOutputStream;
import com.mly.arsc.decode.io.LEDataOutputStream2;
import com.mly.arsc.util.TypedValue;
...
public class ARSCDecoder {
    ...
    private ResBagValue readComplexEntry() throws IOException {
        ...
        for (int i = 0; i < count; i++) {
            items[i] = new Duo<>(mIn.readInt(), (ResScalarValue) readValue());
        }
    ...
    private ResConfigFlags readConfigFlags() throws IOException {
        ...
        if (exceedingSize > 0) {
            ...
            if (exceedingBI.equals(BigInteger.ZERO)) {
                LOGGER.fine(
                        String.format(Locale.ROOT, "Config flags size > %d, but exceeding bytes are all zero, so it should be ok.",
                                KNOWN_CONFIG_BYTES));
            } else {
                LOGGER.warning(String.format(Locale.ROOT, "Config flags size > %d. Exceeding bytes: 0x%X.", KNOWN_CONFIG_BYTES,
                        exceedingBI));
                isInvalid = true;
            }
        }
    ...
    private void readLibraryType() throws IOException {
        ...
        for (int i = 0; i < libraryCount; i++) {
            ...
            packageName = mIn.readNulEndedString(128, true);
            LOGGER.info(String.format(Locale.ROOT, "Decoding Shared Library (%s), pkgId: %d", packageName, packageId));
        }
     ...
     * @throws IOException
     */
    public void write2(File outFile) throws IOException {
        // 先将字符串数据写入到一个临时的流中
        ByteArrayOutputStream mStrings = mTableStrings.writeString(mTableStrings.getList());
        int chunkSize = stringsHeader.chunkSize + (mStrings.size() - mTableStrings.m_strings.length);

        // 二进制文件输出流
        LEDataOutputStream2 lmOut = new LEDataOutputStream2(outFile, chunkSize);
        /////////////////////////////////////////////////////////////////////////////////////////
        // 这里才正式开始写arsc文件
        ...
        // 写入chunkSize
        lmOut.writeInt(chunkSize);
        // 写入包的数量
        lmOut.writeInt(packageCount);
        // 写入字符串
        mTableStrings.writeFully2(lmOut, mStrings);
    ...
    /**
     * 写出arsc文件的方法 os 文件输出流
     *
     * @param stringlist_src 未修改之前的字符串列表集合
     * @param stringlist_tar 修改后的字符串列表集合
     ***/
     ...
     * @param stringlist_tar 修改后的字符串列表集合
     ***/
    public void write(OutputStream os, List<String> stringlist_tar) throws IOException {
        mTableStrings.updateLists(stringlist_tar);
        write(os);
    }
