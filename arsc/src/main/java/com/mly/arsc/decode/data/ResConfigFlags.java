...
package com.mly.arsc.decode.data;
...
public class ResConfigFlags {
    ...
    private String generateQualifiers() {
        ...
        if (mcc != 0) {
            ret.append("-mcc").append(String.format(Locale.ROOT, "%03d", mcc));
            if (mnc != MNC_ZERO) {
                if (mnc != 0) {
                    ret.append("-mnc");
                    if (size <= 32) {
                        if (mnc > 0 && mnc < 10) {
                            ret.append(String.format(Locale.ROOT, "%02d", mnc));
                        } else {
                            ret.append(String.format(Locale.ROOT, "%03d", mnc));
                        }
                    } else {
                        ret.append(mnc);
                    }
        ...
        if (screenWidth != 0 && screenHeight != 0) {
            if (screenWidth > screenHeight) {
                ret.append(String.format(Locale.ROOT, "-%dx%d", screenWidth, screenHeight));
            } else {
                ret.append(String.format(Locale.ROOT, "-%dx%d", screenHeight, screenWidth));
            }
        }
